...
from ..exceptions import GeminiApiError, RateLimitError, ConfigurationError
...
class GeminiClient:
    ...
    def _initialize_models(self):
        """Initialize or reinitialize models with current API key"""
        # Ensure API key is configured before creating models
        genai.configure(api_key=self.keys[self.current_key_index])
        self.models = {
    ...
    def switch_api_key(self):
        ...
        if len(self.keys) > 1:
            ...
            genai.configure(api_key=self.keys[self.current_key_index])
            # Reinitialize models with new API key to ensure proper binding
            self._initialize_models()
    ...
    async def generate_content(self, prompt: str, model_name: str = "flash", 
        ...
        # Ensure API key is configured before each generation attempt
        # This fixes the issue where MCP server calls fail with "API key not valid"
        genai.configure(api_key=self.keys[self.current_key_index])
